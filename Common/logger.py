...
class Logger:
    def __init__(self, agent, **config):
        self.config = config
        self.agent = agent
        self.log_dir = self.config["env_name"] + "/" + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        self.start_time = 0
        self.duration = 0
    ...
    #@staticmethod
    def _create_wights_folder(self, dir):
        if not os.path.exists("Checkpoints/" + self.config["env_name"]):
            os.mkdir("Checkpoints/" + self.config["env_name"])
    ...
    def load_weights(self):
        model_dir = glob.glob("Checkpoints/" + self.config["env_name"]+ "/")
        #model_dir = glob.glob("distilled_skills_model/" + self.config["env_name"] + "/")
        model_dir.sort()
        print(model_dir)
        print(model_dir[-1])
