import numpy as np
from .model import PolicyNetwork, QvalueNetwork, ValueNetwork, Discriminator, DiscretPolicyNetwork
import torch
...
class MetaAgent(nn.Module):
    def __init__(self, **config):
        ...
        self.policy_network = DiscretPolicyNetwork(n_states=self.n_states,
                                            n_actions=self.n_skills,
        ...
        self.value_network = ValueNetwork(n_states=self.n_states,
                                          n_hidden_filters=self.config["n_hiddens"]).to(self.device)

        self.value_target_network = ValueNetwork(n_states=self.n_states,
                                                 n_hidden_filters=self.config["n_hiddens"]).to(self.device)
