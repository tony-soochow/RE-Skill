...
class SACAgent:
    ...
    def train(self):
        if len(self.memory) < self.batch_size:
            ...
            logits = self.discriminator(torch.split(next_states, [self.n_states, self.n_skills], dim=-1)[0]) #输出对每个q(z|s)的原始预测值
            
            p_z = p_z.gather(-1, zs)
            logq_z_ns = log_softmax(logits, dim=-1) #将原始预测值转换为对数概率分布
