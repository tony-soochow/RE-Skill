...
class IncrementalAgent(Agent,nn.Module):
    ...
    def save_model(self, file_path):
        # 创建一个包含模型参数的 state_dict
        state = {
            'model_parameters': self.state_dict(),
            'skill_actors': self.skill_actors
        }
        # 保存整个模型（包括state_dict和其他属性）到文件
    ...
    def load_model(self, file_path):
        ...
        # 提取模型参数和skill_actors
        self.load_state_dict(loaded_model["model_parameters"])
        self.skill_actors = loaded_model["skill_actors"]
