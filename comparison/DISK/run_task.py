...
from disk_meta_env_wrapper import meta_env_wrapper
import torch
...
from agent.incremental_agent import IncrementalAgent
...
def run(frames=1000, eps_fixed=False, eps_frames=1e6, min_eps=0.01, eval_every=1000, eval_runs=20, worker=1):
    ...
    log_dir = "logs-disk/" + args.env + "-" + time + ".txt"
    f = open(log_dir, "w")
    for frame in tqdm(range(1, frames+1)):
        action = agent.act(state, eps)
        next_state, reward, done, _ = envs.task_step(action)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env_name", type=str, default="Hopper-v3",help="Name of skill_env")
    ...
    #     'device': 'cuda',
    #     'critic_cfg': {'_target_': 'agent.critic.DoubleQCritic', 'obs_dim': 11, 'action_dim': 3, 'hidden_dim': 1024, 'hidden_depth': 2},
    #     'actor_cfg': {'_target_': 'agent.actor.DiagGaussianActor', 'obs_dim': 11, 'action_dim': 3, 'hidden_depth': 2, 'hidden_dim': 1024, 'log_std_bounds': [-5, 2]},
    #     'discount': 0.99,
    ...
    agent_config =  {
        ...
        'action_range': [-1.0, 1.0], 
        'device': 'cuda', 
        'critic_cfg': {'_target_': 'agent.critic.DoubleQCritic', 'obs_dim': 17, 'action_dim': 6, 'hidden_dim': 1024, 'hidden_depth': 2}, 
        'actor_cfg': {'_target_': 'agent.actor.DiagGaussianActor', 'obs_dim': 17, 'action_dim': 6, 'hidden_depth': 2, 'hidden_dim': 1024, 'log_std_bounds': [-5, 2]}, 
        'discount': 0.99, 
        'init_temperature': 0.1, 
        'alpha_lr': 0.0001, 
        'alpha_betas': [0.9, 0.999], 
    ...
    lower_level_agent = IncrementalAgent(**agent_config)
    lower_level_agent.load_model("Checkpoints/"+args.env_name+"/params.pth")
    # print(lower_level_agent.skill_actors)
    # lower_level_agent = torch.load("Checkpoints/"+args.env_name+"/params.pth")
